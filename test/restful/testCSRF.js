...
app_CSRF.get('/', function (req, res) {
    res
        .status(200)
        .send('<html><body>Attempt to send request from a separate origin:<form method="POST" action="http://localhost:3000/users/1"><input type="submit"\></form></body></html>')
        // .end();
    return;
});
app_CSRF.listen(3001, function () {
    console.log('Same-Site CSRF mitigation demo app running on localhost:30001');
})
/*
